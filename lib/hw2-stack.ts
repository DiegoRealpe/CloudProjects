...
export class ImagebuilderPipeline extends Construct {
	...
	private createKMSKey(): kms.Key {
		const givenKeyAlias = this.node.tryGetContext('givenKeyAlias') ?? undefined;
	...
	constructor(scope: Construct, id: string) {
		...
		let ec2EncryptionKey: kms.Key;

		let keyid;

		if (this.node.tryGetContext('shouldUseAMIEncryption') === undefined) {
			keyid = undefined;
		} else {
			ec2EncryptionKey = this.createKMSKey();
			keyid = ec2EncryptionKey.keyId;
		}
