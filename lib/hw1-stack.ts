...
export class Hw1Stack extends cdk.Stack {
	constructor(scope: Construct, id: string, props?: cdk.StackProps) {
		...
		const securityGroupNameParam = this.node.tryGetContext(
		...
		const givenKey = ec2.KeyPair.fromKeyPairName(
			this,
			'givenKey',
			keyNameParam,
		);
		...
		try {
			givenSecurityGroup = ec2.SecurityGroup.fromLookupByName(
				this,
				'givenSecurityGroup',
				securityGroupNameParam,
				vpc,
			...
			givenSecurityGroup = new ec2.SecurityGroup(this, 'sshSecurityGroup', {
				vpc: vpc,
